...
static void mtk_wdt_mode_config(	BOOL dual_mode_en, 
					...
					BOOL ext_pol, 
					BOOL wdt_en );
int mtk_wdt_hw_reboot_fail_check(void)
{
	int status =0;
	int ret_val =0;
	int reg_val =0;
	int tmp = 0;
	int wdt_kernel_init_success=0;
	int wdt_hw_reboot_fail_count=0;
	print("wdt:mtk_wdt_hw_reboot_fail_check++ 0x(%x)\n ",DRV_Reg32(MTK_WDT_NONRST_REG2));
	status = DRV_Reg32(MTK_WDT_STATUS);
	if(status & MTK_WDT_STATUS_HWWDT_RST)
	{
		wdt_kernel_init_success = (DRV_Reg32(MTK_WDT_NONRST_REG2)&0x04);
		if(wdt_kernel_init_success)
		{
		 	//kerne init ok or first init.
		 	//clear hw reboot fail count
			tmp = DRV_Reg32(MTK_WDT_NONRST_REG2);
			tmp = tmp&0xff87; //10000111b clear bit3~bit 6
			DRV_WriteReg32(MTK_WDT_NONRST_REG2,tmp);
			//clrear init sucess flag
			tmp = DRV_Reg32(MTK_WDT_NONRST_REG2);
			tmp = tmp&0xfffb; //11111011b clear bit2
			DRV_WriteReg32(MTK_WDT_NONRST_REG2,tmp);
			udelay(100);
			print("wdt:wdt_kernel_init_success & hw reboot 0x(%x) \n",DRV_Reg32(MTK_WDT_NONRST_REG2));
		}
		else
		{
			//hw reboot fail count
			wdt_hw_reboot_fail_count = (DRV_Reg32(MTK_WDT_NONRST_REG2)&0x78)>>0x3;//read bit3~bit6
			wdt_hw_reboot_fail_count++;
			tmp = DRV_Reg32(MTK_WDT_NONRST_REG2);
			tmp = tmp&0xff87; //10000111b clear bit3~bit 6
			tmp = tmp | (wdt_hw_reboot_fail_count << 3);
			DRV_WriteReg32(MTK_WDT_NONRST_REG2,tmp);
			
			if(wdt_hw_reboot_fail_count>=HW_RESET_FAIL_MAX_COUNT)
			{
				////power down reboot config
                ret_val=pmic_read_interface( 0x011A, (&reg_val),0xFFFF, 0);
                print("[pmic_init] 3.RGU reset,Reg[0x%x]=0x%x\n", 0x011A, reg_val);
                ret_val=pmic_config_interface(0x011A, 0x0, 0x1, 0); //add this line
                ret_val=pmic_read_interface( 0x011A, (&reg_val),0xFFFF, 0);
                print("[pmic_init] 4.RGU reset,Reg[0x%x]=0x%x\n", 0x011A, reg_val);

                ////set diglitch =0;
                DRV_WriteReg32(MTK_WDT_DEGLITCH_EN,0);
                ////sw reset
                print("wdt:  hw reboot fail!!! count=%d, REG2=0x(%x) \n",wdt_hw_reboot_fail_count,DRV_Reg32(MTK_WDT_NONRST_REG2));
                udelay(1000);
				//set magic numer tell preloader this is special reboot
				 *(volatile u32 *)( RAM_CONSOLE_ADDR )= WDT_RESET_MAGIC_NUM;
                mtk_arch_reset(1);
				
			}
			else
			{
				print("wdt: after hw reboot fail, count=%d, REG2=0x(%x) \n",wdt_hw_reboot_fail_count,DRV_Reg32(MTK_WDT_NONRST_REG2));
			}
		}
	}
	else
	{
		...
		DRV_WriteReg32(MTK_WDT_NONRST_REG2,tmp);
		udelay(100);
		print("no hw reboot 0x(%x) \n",DRV_Reg32(MTK_WDT_NONRST_REG2));
	}
	print("mtk_wdt_hw_reboot_fail_check-- 0x(%x) \n",DRV_Reg32(MTK_WDT_NONRST_REG2));
	return 0;
...
int mtk_wdt_boot_check(void)
{
    ...
    if (wdt_sta & (MTK_WDT_STATUS_HWWDT_RST|MTK_WDT_STATUS_SWWDT_RST|MTK_WDT_STATUS_THERMAL_RST|MTK_WDT_STATUS_SPMWDT_RST)) {
        if (rgu_mode & MTK_WDT_AUTO_RESTART_FLAG) {
...
void mtk_wdt_init(void)
{
	...
	print("RGU MTK_WDT_NONRST_REG2: %x\n", DRV_Reg32(MTK_WDT_NONRST_REG2));
	print("RGU MTK_WDT_NONRST_REG: %x\n", DRV_Reg32(MTK_WDT_NONRST_REG));
    print("==== Dump RGU Reg End ====\n");

	#ifdef HW_RESET_FAIL_CONFIG
	mtk_wdt_hw_reboot_fail_check();
	#endif 
    rgu_mode = DRV_Reg32(MTK_WDT_MODE);
	wdt_non_reg2 = DRV_Reg32(MTK_WDT_NONRST_REG2);
	if(wdt_non_reg2 & MTK_WDT_AUTO_RESTART_FLAG)
	{
		rgu_mode |=MTK_WDT_AUTO_RESTART_FLAG;
	}
	else
	{
		rgu_mode&=(~MTK_WDT_AUTO_RESTART_FLAG);
	...
	#else
	if ((wdt_sta & MTK_WDT_STATUS_HWWDT_RST)&&(rgu_mode&MTK_WDT_AUTO_RESTART_FLAG)) 
	{ /* For E1 bug, that SW reset value is 0xC000, we using "==" to check */
    	/* Time out reboot always by pass power key */
        g_rgu_status = RE_BOOT_BY_WDT_HW;
