...
#if defined(MTK_BQ24158_SUPPORT)
	bq24158setDefaultV();//<battery locked>
#endif
	if (reg_val == 1)
    {                     
        printf("No Battery\n");
...
U32 pmic_init (void)
{
    ...
    print("[pmic_init] PMIC CHIP Code = 0x%x\n", get_pmic_chip_version());
#if defined(MTK_BQ24158_SUPPORT)
	bq24158setDefaultV();
#endif
    ...
    print("[pmic_init] PowerKey = %d\n", pmic_detect_powerkey());    
    print("[pmic_init] is USB in = 0x%x\n", pmic_IsUsbCableIn());
    ...
    print("[pmic_init for DUMMY_AP] Reg[0x%x]=0x%x\n", 0x30E, reg_val);
    #endif
...
#define I2C_NUM   1
void bq24158setDefaultV(void)
{       
	U8 write_data[8];
	int ret=0;
	int dataBuffer=0;
	static struct mt_i2c_t bq24158_i2c;
	print("[bq24158setDefaultV] bq24158setDefaultV\n");
#if 1
	bq24158_i2c.id = I2C1;
	...
	bq24158_i2c.speed = 400;
	dataBuffer=6;
	ret= i2c_write_read(&bq24158_i2c, &dataBuffer, 1, 1);
	printf("[bq24158setDefaultV] : i2c_read 0x6: ret: %d ,buf:0x%x \n", ret,dataBuffer);
	...
	if(dataBuffer==0x77)
	{
		...
		write_data[0]=0x2;	
		write_data[1]=0xaa;
		ret=i2c_write(&bq24158_i2c, write_data, 2);
		print("[bq24158setDefaultV] set reg 2 ret=%d\n",ret);
			...
			write_data[1]=0x77;
			ret=i2c_write(&bq24158_i2c, write_data, 2);
			print("[bq24158setDefaultV] set reg 6 ret=%d\n",ret);
...
#endif//END<20130708><battery locked>wangyanhui
