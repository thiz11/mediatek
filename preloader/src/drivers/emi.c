...
#if COMBO_PCDDR3 > 0
#ifdef DDR3_16B
	print("mt6592 ddr3 x16 init : \n");
#else
	print("mt6592 ddr3 x32 init : \n");
#endif
...
#define Vcore_HV_LPPDR3  (0x54)   //1.225
#define Vcore_NV_LPPDR3  (0x44)   //1.125
#define Vcore_LV_LPPDR3  (0x37)   //1.045
...
#define Vmem_LV_LPDDR2 (0x4A) //1.14
...
void DRAM_Vmem_adjust(int nAdjust)
{
    ...
    unsigned int OldVmem2 = 0;
		...
		case 7 :		// PCDDR3 HV. Vmem 1.418V
				mt6333_config_interface(0x7F, 0x40, 0xC0,0);
...
else if ((emi_set->type & 0xF) == TYPE_LPDDR3)
{
    ...
    #endif
    print("0xF4 0x%x\n",DRAMC_READ_REG(0xf4));
    DRAMC_WRITE_REG(DRAMC_READ_REG(0xf4)|0x02000000,0xF4);
    print("0xF4 0x%x\n",DRAMC_READ_REG(0xF4));
    DRAMC_WRITE_REG(DRAMC_READ_REG(0xf4)& (~0x02000000),0xF4);
    print("0xF4 0x%x\n",DRAMC_READ_REG(0xF4));
    print("0xF0 0x%x\n",DRAMC_READ_REG(0xF0));
    DRAMC_WRITE_REG(DRAMC_READ_REG(0xF0)|0x01000000,0xF0);
    print("0xF0 0x%x\n",DRAMC_READ_REG(0xF0));
    DRAMC_WRITE_REG(DRAMC_READ_REG(0xF0)&~(0x01000000),0xF0);
    print("0xF0 0x%x\n",DRAMC_READ_REG(0xF0));
    init_lpddr3(emi_set);
...
{//fy for 92 bring up
	...
	DRAM_Vcore_adjust(7);
	DRAM_Vmem_adjust(7);
	print("[EMI] PCDDR3 HV :Vcore = 1.15V, Vmem = 1.418V\r\n");
	#elif VcLV_VmLV
	DRAM_Vcore_adjust(9);
	DRAM_Vmem_adjust(9);
	print("[EMI] PCDDR3 LV :Vcore = 1.00V, Vmem = 1.283V\r\n");
	#else
	DRAM_Vcore_adjust(8);//Because PCDDR3 power on at DDR3-1466,we set vcore to 1.125V
	DRAM_Vmem_adjust(8);
	print("[EMI] PCDDR3 NV :Vcore = 1.125V, Vmem = 1.350V\r\n");
	#endif
	#endif
	//2.mempll 1066MHz --> 1466MHz
	int dram_clock = 1466;
...
//    print_DBG_info();
    ...
    *((V_UINT32P)(DDRPHY_BASE + 0x01dc)) &= ~(0x1 << 30);
    ...
    //pmic_voltage_read(1);//check the pmic setting
#ifdef DMA_TEST
//Example for tes 1GB DRAM. DMA TEST DRAM with 1MB each time.
int DRAM_SIZE=0x80000000;
int rank=2;
int DMA_SIZE=1024*1024;
int DMA_START_ADDRESS=0x80000000;
int i;
int dma_ret;
print("DRAM-DMA CROSS RANK TEST START\n");
      while(1)
     {
        mtk_wdt_disable();
       for(i=0;i<DRAM_SIZE/DMA_SIZE;i++){
       dma_ret= dramc_dma_test_local(DMA_START_ADDRESS+i*DMA_SIZE,DMA_SIZE,rank,DMA_START_ADDRESS,DRAM_SIZE);
       if(dma_ret==-1){
          ASSERT(0);
       }
       }
...
int dramc_ta2_test(void)
{
    ...
    DRAMC_WRITE_REG(0x55000000|0x800000,0x3C);
    data = DRAMC_READ_REG(0x40);
    DRAMC_WRITE_REG((data & 0xAA000000) | 0x00FFFFFF, 0x40);
    DRAMC_WRITE_SET((1 << 31),DRAMC_CONF2); //Test Agent 2 write enabling
    DRAMC_WRITE_SET((1 << 30) | (1 << 31),DRAMC_CONF2); //Test Agent 2 write enabling, Test Agent 2 read enabling
...
#endif
#ifdef DMA_TEST
int dramc_dma_test_local(unsigned int start, unsigned int len,int rank,int dram_start_addr,int dram_size){
    int err = 1;
    unsigned int data;
    int i;
    unsigned int *src_buffp1;
    unsigned int *dst_buffp1;
    unsigned int rank0_addr=0;
    unsigned int rank1_addr=0;
    if(rank==2)
    {
      rank0_addr=dram_start_addr;
      rank1_addr=dram_start_addr+dram_size/2;
      if(start>=rank1_addr)
      {
        src_buffp1=start;
        dst_buffp1=(start-rank1_addr)+rank0_addr;
      }
      else
      {
        src_buffp1=start;
        dst_buffp1=(start-rank0_addr)+rank1_addr;
      }
    ...
    {
      src_buffp1 = start;
      dst_buffp1 = start+len;
       
      if(dst_buffp1>=dram_start_addr+dram_size)
      {
        dst_buffp1=dram_start_addr;
      //  print("wrap to 0x%x\n",dst_buffp1);
      }
    }
     ...
     *((volatile unsigned int *)(0x110000A4)) = len-1;
     *((volatile unsigned int *)(0x11000088)) = 0x1; //start dma
     while(*((volatile unsigned int *)(0x11000088)));

//    print("0x%x:0x%x\n",0x11000098, *((volatile unsigned int *)(0x11000098)));
//    print("0x%x:0x%x\n",0x1100009c, *((volatile unsigned int *)(0x1100009C)));
//    print("0x%x:0x%x\n",0x110000A0, *((volatile unsigned int *)(0x110000A0)));
//    print("0x%x:0x%x\n",0x110000A4, *((volatile unsigned int *)(0x110000A4)));
    ...
    print("src:0x%x to dst:0x%x OK! \n",src_buffp1,dst_buffp1);
    if(src_buffp1==rank1_addr)
    {
      print("rank 0 pass DMA Test\n");
    }
    if(src_buffp1==(dram_start_addr+dram_size)-len)
    {
      print("rank 1 pass DMA Test\n");
    }
    return err;
