...
void platform_set_boot_args()
{
    ...
    bootarg.ddr_dfs_enable =  DFS_ability_detection();
...
static void platform_emacro_powerdown()
{
    U32 power_down_pin = 0x10206230;
   /*Turn off Power PIN*/
    *((P_U32)power_down_pin) |= (1<<24);
}
...
void platform_pre_init(void)
{
    ...
    /*MT6592 Emacro power down*/
    platform_emacro_powerdown();
...
//enable long press reboot function***************
#if !CFG_EVB_PLATFORM
	// Jake.L, DATE20140219, TINNO_A_S0015, Time of long pressed controlled by KPD_PMIC_LPRST_TD, DATE20140219-01 START
	#if defined (ONEKEY_REBOOT_NORMAL_MODE_PL)
	...
	#elif defined (TWOKEY_REBOOT_NORMAL_MODE_PL)
			pmic_config_interface(TOP_RST_MISC, 0x01, PMIC_RG_PWRKEY_RST_EN_MASK, PMIC_RG_PWRKEY_RST_EN_SHIFT);
