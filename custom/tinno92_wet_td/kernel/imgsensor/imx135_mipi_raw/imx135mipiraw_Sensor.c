...
#define IMX135_LSC_OTP_Enable 0
#if IMX135_LSC_OTP_Enable 
...
#if IMX135_LSC_OTP_Enable
void IMX135OTPWriteSensor(kal_uint16 address, kal_uint16 para)
{
	IMX135MIPI_write_cmos_sensor(address,para);

}

kal_uint16 IMX135OTPReadSensor(kal_uint32 address)
{
	kal_uint16 get_byte=0;
	get_byte=IMX135MIPI_read_cmos_sensor(address);
	return get_byte;
}

kal_uint8 IMX135_goto_page(kal_uint8 page)
{
	kal_uint8 rdrdy_status = 0;
	kal_uint8 checktimes = 2,i = 0;
	for(i= 0; i<checktimes;i++)
	{
		IMX135OTPWriteSensor(0x3b02, page);
    	IMX135OTPWriteSensor(0x3b00, 0x01);
    	mdelay(2);
    	rdrdy_status = IMX135OTPReadSensor(0x3b01);
		if( rdrdy_status == 1)
		{			
			return 1;
		}
...
kal_uint8 check_IMX135_otp_valid_LSC_Page(void)
{
	kal_uint8 Pagemax = 12;
	kal_uint8 Pagemin = 9;
	kal_uint8 page = 0;
	kal_uint16 LSC_OK = 0x00ff;
	for(page = Pagemax ; page>=Pagemin;page--)
	{
	if (IMX135_goto_page(page))
		{
			LSC_OK = IMX135OTPReadSensor(0x3b0f);
			if (LSC_OK == 0)
			{
				SENSORDB("can read LSC otp from page1~page8\n");
				SENSORDB("page number %d is valid\n",page);
				break;
			}		
		}
	}
	if ( page<9)
		{		
		SENSORDB("No LSC OTP data!");
		return 0;
		}
	return page;
}

kal_bool IMX135_Read_LSC_Otp(kal_uint8 pagestart,kal_uint8 pageend)
{

kal_uint8 page = 0;
kal_uint8 byteperpage = 64;
kal_uint8 number = 0;
kal_uint16 LSCOTPaddress = 0x3b04 ; 
int i = 0;
page = check_IMX135_otp_valid_LSC_Page();
if(page)
{
	SENSORDB("LSC OTP exist! Valid page is page:%d\n",page);
}
else{
	SENSORDB("no LSC otp\n");
return KAL_FALSE;
...
for(page = pagestart; page<=pageend; page++)
{
	...
	{
	lscotpdata[i+number*64]=IMX135OTPReadSensor(LSCOTPaddress+i);
	//SENSORDB("lscotpdata[%d] = %d\n",(i+number*64),lscotpdata[i+number*64]);
	}
	number ++;	
}
	return KAL_TRUE;
...
void IMX135_Update_Lsc(kal_uint8 * dataBuf)
{
	int i;	
	SENSORDB("IMX135_Update_Lsc enter:\n");
		for(i = 0; i <LSCOTPDATASIZE-8; i++)
		{
			//JRDSENSORDB("decompressData[%d] = %d\n", i, dataBuf[i]);
			IMX135OTPWriteSensor((0x4800+i),dataBuf[i]);
		}
	
		IMX135OTPWriteSensor(0x4500, 0x1f);
		IMX135OTPWriteSensor(0x0700, 0x01);
		IMX135OTPWriteSensor(0x3a63, 0x01);
		SENSORDB("IMX135_Update_Lsc exit:\n");
}
#endif
...
//quality
   ...
   // ARNR Parameter setting
	...
	#if IMX135_LSC_OTP_Enable
	if(IMX135_Read_LSC_Otp(1,8))
	{
		IMX135_Update_Lsc(lscotpdata);
		SENSORDB("LSC OTP update done \n");
	}
	else
	{
		SENSORDB("LSC OTP is not exist \n");
	}
	#endif
