...
static void imx135_wordwrite_cmos_sensor(u16 addr, u32 para){
    iWriteReg((u16)addr, (u32)para, 2, IMX136MIPI_WRITE_ID);
}

static void imx135_bytewrite_cmos_sensor(u16 addr, u32 para){
    iWriteReg((u16)addr, (u32)para, 1, IMX136MIPI_WRITE_ID);
}
...
static bool otp_update()
{
	...
	if((MID != TRULY_ID)&&(MID != TRULY_ID_BYPASS)) //Select 
	{
...
bool is_truly_cm9010(void){
	...
	int i;
      ...
      ret = IMM_GetOneChannelValue_Cali(1, &camera_id_volt);
      if(ret != 0)
      ...
      camera_id_volt/=1000;          //volt uV to mV
       if ((camera_id_volt < 600 )||(camera_id_volt > 1200) )			//truly ID volt is 0.9V. shunyu is 0V
       {
   		printk("camera id volt error ,camera_id_volt =%d\n",camera_id_volt);
		return 0;
       }
