...
int read_otp(uint page, uint32_t address, uint* array, uint32_t size)
{
    ...
    while (j<size) {
        ...
        {
            #ifdef IMX135_OTP_DEBUG
            printk("read_otp:Status check : 0x3B01 == 0x01, read ready (bit0 1:read ready)\n");
            #endif
        }
        else
        {
            printk(" read_otp:Status check : 0x3B01 != 0x01, read not ready (bit0 1:read ready)\n");
            return 1;
        }
        while(i<64)
        {
...
uint32_t RG_Ratio_typical_Sunny=0x0242;
uint32_t BG_Ratio_typical_Sunny=0x0268;
uint32_t GG_Ratio_typical_Sunny=0x0400;
...
int update_awb_gain()//uint32_t R_gain, uint32_t G_gain, uint32_t B_gain)
{
	...
	printk("[Sunny] update_awb_gain() call, wb_flag == 0.\n");
	 ...
	 #endif
	IMX135MIPI_write_cmos_sensor(0x020E, G_gain1_R>>8);
	IMX135MIPI_write_cmos_sensor(0x020F, G_gain1_R& 0xFF);
	IMX135MIPI_write_cmos_sensor(0x0210, R_gain1 >>8);
	IMX135MIPI_write_cmos_sensor(0x0211, R_gain1 & 0xFF);
	IMX135MIPI_write_cmos_sensor(0x0212, B_gain1 >>8);
	IMX135MIPI_write_cmos_sensor(0x0213, B_gain1 & 0xFF);
	IMX135MIPI_write_cmos_sensor(0x0214, G_gain1_B>>8);
	IMX135MIPI_write_cmos_sensor(0x0215, G_gain1_B& 0xFF);
        return 0;
    }
}
...
int get_sensor_customer_id()
{
   ...
   if(ret != 0)
	    printk("[get_sensor_imx135_customer_id]camera_id_volt read fail\n");
    else
	    printk("[get_sensor_imx135_customer_id]camera_id_volt = %d\n", camera_id_volt);
   ...
   camera_id_volt/=1000;          //volt uV to mV
    ...
    IMX135MIPI_write_cmos_sensor(0x0100, 0x0);
...
//	customer_id = current_otp.customer_id;
    ...
    printk("get_sensor_customer_id =%d\n",customer_id);
