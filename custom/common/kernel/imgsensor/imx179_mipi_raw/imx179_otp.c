...
static int check_otp_awb(uint index)
{
   ...
   while(check_time--)
   {
	    temp = IMX179MIPI_read_cmos_sensor(0x3401);
	    if ((temp&0x01) ==0x1)
	    {
...
static int read_otp(uint page, uint32_t address, uint* array, uint32_t size)
{
    ...
    while (j<size) {
   	...
   	if(check_time == 0)
        ...
        while(i<64)
        {
            *(array+j) = IMX179MIPI_read_cmos_sensor(address+i);
            #ifdef IMX179_OTP_DEBUG
...
static int read_otp_wb(uint index, struct otp_struct* otp)
{
    ...
    page = 0x00 + index;    
/*
    IMX179MIPI_write_cmos_sensor(0x3B02, page);
    // turn on otp read mode
    IMX179MIPI_write_cmos_sensor(0x3B00, 0x01);
    mdelay(100);
    // Status check : 0x3B01 = 0x01 (bit0 1:read ready)
    temp = IMX179MIPI_read_cmos_sensor(0x3B01);
    if ((temp&0x01) ==0x01)
    {
    ...
    otp->year = IMX179MIPI_read_cmos_sensor(address+3);
    otp->month = IMX179MIPI_read_cmos_sensor(address+4);
    otp->day = IMX179MIPI_read_cmos_sensor(address+5);
    otp->customer_id = IMX179MIPI_read_cmos_sensor(address+6);
    otp->lens_id = IMX179MIPI_read_cmos_sensor(address+7);
    otp->vcm_id = IMX179MIPI_read_cmos_sensor(address+8);
    otp->rg_ratio = (IMX179MIPI_read_cmos_sensor(address+15)<<8)|IMX179MIPI_read_cmos_sensor(address+16);
    otp->bg_ratio = (IMX179MIPI_read_cmos_sensor(address+17)<<8)|IMX179MIPI_read_cmos_sensor(address+18);
    otp->gg_ratio = (IMX179MIPI_read_cmos_sensor(address+19)<<8)|IMX179MIPI_read_cmos_sensor(address+20);
*/
...
/*
    while (j<384) {
        ...
        {
            address = 0x3B04;
            arr[j] = IMX179MIPI_read_cmos_sensor(address+i);
...
int imx179_update_awb_gain()//uint32_t R_gain, uint32_t G_gain, uint32_t B_gain)
{
	...
	printk("[Sunny] update_awb_gain() call, wb_flag == 0.\n");
	 ...
	 #endif
	IMX179MIPI_write_cmos_sensor(0x020E, G_gain1_R>>8);
	IMX179MIPI_write_cmos_sensor(0x020F, G_gain1_R& 0xFF);
	IMX179MIPI_write_cmos_sensor(0x0210, R_gain1 >>8);
	IMX179MIPI_write_cmos_sensor(0x0211, R_gain1 & 0xFF);
	IMX179MIPI_write_cmos_sensor(0x0212, B_gain1 >>8);
	IMX179MIPI_write_cmos_sensor(0x0213, B_gain1 & 0xFF);
	IMX179MIPI_write_cmos_sensor(0x0214, G_gain1_B>>8);
	IMX179MIPI_write_cmos_sensor(0x0215, G_gain1_B& 0xFF);
        return 0;
    }
}

/*
int imx179_update_lens()//(struct otp_struct otp)
{
	...
	printk("[Sunny] update_lens() call, lenc_flag == 0.\n");
	#endif
        //Turn off lsc
        IMX179MIPI_write_cmos_sensor(0x4500, 0x00);
        IMX179MIPI_write_cmos_sensor(0x0700, 0x00);
        IMX179MIPI_write_cmos_sensor(0x3A63, 0x00);

        //Access LSC table //table1
        for(i=0;i<504;i++) {
            IMX179MIPI_write_cmos_sensor(0x4800+i, current_otp2.lenc[i]);
            #ifdef IMX179_OTP_DEBUG
        ...
        //Turn on lsc
        IMX179MIPI_write_cmos_sensor(0x4500, 0x1f);
        IMX179MIPI_write_cmos_sensor(0x0700, 0x01);
        IMX179MIPI_write_cmos_sensor(0x3A63, 0x01);
...
// return value: 0 update success
//               1, no OTP
/*int imx179_update_otp_lenc(void)
{
...
int get_sensor_imx179_customer_id()
{
 ...
 /*   int camera_id_volt = 0;
   ...
   mt_set_gpio_mode(GPIO115|0x80000000,GPIO_MODE_00);	
   mt_set_gpio_dir(GPIO115|0x80000000,GPIO_DIR_IN);
   if(GPIO_OUT_ONE == mt_get_gpio_in(GPIO115|0x80000000))
   	customer_id=2;	
   else		
   	customer_id=1;
