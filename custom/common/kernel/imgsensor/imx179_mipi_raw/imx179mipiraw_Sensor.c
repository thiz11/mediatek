...
#define IMX179_USE_AWB_OTP
static uint16_t used_otp = 0;
extern imx179_update_awb_gain();
extern imx179_update_otp_wb();
extern get_sensor_imx179_customer_id();
#endif
...
UINT32 IMX179MIPIGetSensorID(UINT32 *sensorID) 
{
    ...
    // check if sensor ID correct
    do {		
	   sensor_id =(kal_uint16)(((IMX179MIPI_read_cmos_sensor(0x0002)&&0x0f)<<8) | IMX179MIPI_read_cmos_sensor(0x0003)); 

	  if (sensor_id == IMX179MIPI_SENSOR_ID)  
        {

		
	     customer_otp_id =get_sensor_imx179_customer_id();			 //customer_otp_id 			shunyu:1  truly:2
//	     if(customer_otp_id == 2){
	            *sensorID = IMX179MIPI_SENSOR_ID;
	            break;
//	     }
        }
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	kal_uint16 iStartX = 0, iStartY = 0;
	   ...
	   if(IMX179MIPI_sensor.capture_mode==KAL_TRUE)
                   return ERROR_NONE;
