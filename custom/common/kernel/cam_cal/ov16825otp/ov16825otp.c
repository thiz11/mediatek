...
extern kal_uint16 HI704_read_cmos_sensor(kal_uint8 addr);
...
int iReadCAM_CAL(u16 a_u2Addr, u32 ui4_length, u8 * a_puBuff)
{
    int  i4RetValue = 0;
    u8  Device_id;
	if((a_u2Addr >= 0xA000)&&(a_u2Addr <= 0xA0FF ))
		{
			Device_id = 0xA0;
		}
	else if((a_u2Addr >= 0xA200)&&(a_u2Addr <= 0xA2FF ))
	...
	else if((a_u2Addr >= 0xA400)&&(a_u2Addr <= 0xA4FF ))
		{
			Device_id = 0xA4;
		}
	else if((a_u2Addr >= 0xA600)&&(a_u2Addr <= 0xA6FF ))
		{
			Device_id = 0xA6;		
		}
	//else if((a_u2Addr >= 0xA300)&&(a_u2Addr <= 0xA3FF))
	...
	*a_puBuff = get_byte;
	...
	CAM_CALDB("[CAM_CAL test] Device_id is %x \n",Device_id);

	//puSendCmd = (char)(a_u2Addr+1);
    //iReadRegI2C(&puSendCmd , 1, (u8*)&get_byte,1,	Device_id);
	//*a_puBuff = get_byte;
...
static int iWriteData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   ...
   CAM_CALDB("[CAM_CAL] iWriteData pinputdata is %x \n",pinputdata[0]); 
...
static unsigned short force[] = {CAM_CAL_I2C_GROUP_ID, ov16825OTP_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END};   
#else
//static unsigned short force[] = {IMG_SENSOR_I2C_GROUP_ID, ov16825OTP_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END};   
#endif
