...
//add this file by xiaoguang.xiong,2013.11.27,for open/short test and rawdata show.CR 562473.
...
#define GTP_TEST_INFO_MAX           300//200 // test info lines max count
...
struct kobject *goodix_ctp_kobj;
...
//add by xiaoguang.xiong,2013.11.24,for raw data show function.
#define SET_INFO_LINE_INFO_2(fmt, args...)       do{ memset(tmp_info_line, '\0', 80);\
...
u8 tp_module_info = 2;//  2 for junda   1 for yeji
u8 set_tpmoduleinfo(u8 val)
{
    tp_module_info =val;
    GTP_INFO("set_tpmoduleinfo tp_module_info is : %d\n",tp_module_info);
}

u8 get_tpmoduleinfo(void)
{
    GTP_INFO("get_tpmoduleinfo tp_module_info is : %d\n",tp_module_info);
    return tp_module_info;
}

//begin,xiaoguang.xiong,2013.11.24,for raw data show.
s32 gt9xx_rawdata_show(struct i2c_client * client)
{
    u16 i = 0;
    u16 j = 0;
    u16 k = 0;
    ...
    //SET_INFO_LINE_INFO("Driver num: %d, Sensor num: %d.\n", gt9xx_drv_num, gt9xx_sen_num);
    ...
    //begin,shenhuijun 20140114 for rd_result
    // u16 max_limit_value = 1800;     // screen max limit
    // u16 min_limit_value = 1000;     // screen min limit
    // u16 max_limit_key = 1500;       // key_val max limit
    // u16 min_limit_key = 800;
    u16 max_tp_line_head = 1190;//1058;
    u16 min_tp_line_head = 450;//603;
    u16 max_tp_va = 2083;//1849;
    u16 min_tp_va = 728;//935;
    u16 max_tp_key = 625;
    u16 min_tp_key = 248;//408;

    GTP_INFO("Rawdata tp_module_info is %d \n",tp_module_info);
    if (1 == get_tpmoduleinfo()){  // for yeji module
        max_tp_line_head = 1826;     
        min_tp_line_head = 491;
        max_tp_va = 2826;          
        min_tp_va = 785;
        max_tp_key = 965;
        min_tp_key = 300;
    }
    else {     // if sensor id = 2 for junda module
        max_tp_line_head = 1190;//1058;
        min_tp_line_head = 450;//603;
        max_tp_va = 2083;//1849;
        min_tp_va = 728;//935;
        max_tp_key = 625;
        min_tp_key = 248;//408;
    ...
    {
        for (k = 0; k < gt9xx_sen_num; k++,raw_buf++)
        {
            if(make_rd == 1)
            {
                if((j == 20) && ((k == 1) || (k == 5) || (k == 8)))
                {
                    if((*raw_buf < min_tp_key) || (*raw_buf > max_tp_key))
//                        SET_INFO_LINE_INFO_2("row:%d,column:%d,err_key:%d \n", j,k,*raw_buf);
                        value_check = 1;
                    continue;
                }
                ...
                if((j == 0) || (j == 19))
                {
                    if((*raw_buf < min_tp_line_head) || (*raw_buf > max_tp_line_head))
                    {
//                        SET_INFO_LINE_INFO_2("row:%d,column:%d,err_data:%d \n", j,k,*raw_buf);
                        value_check = 1;
                    }
                }
                else if ((j > 0) && (j < 19 ))
                {
                    if((*raw_buf < min_tp_va) || (*raw_buf > max_tp_va))
                    {
//                        SET_INFO_LINE_INFO_2("row:%d,column:%d,err_data:%d \n", j,k,*raw_buf);
                        value_check = 1;
                    }
                ...
                SET_INFO_LINE_INFO_2("%d ", *raw_buf);
        }
        if(make_rd != 1)
            SET_INFO_LINE_INFO_2("\n");
    }

    if(make_rd == 1)
    {
        if (value_check == 1)
            SET_INFO_LINE_INFO_2("FAIL");
        else
            SET_INFO_LINE_INFO_2("PASS");
    }

    for (j = 0; j < gt9xx_drv_num; j++)
    {
        for (k = 0; k < gt9xx_sen_num; k++)
        {
            raw_buf--;
        }
    }
...
//end,xiaoguang.xiong,2013.11.24,for raw data show.
...
static ssize_t gtp_sysfs_rawdata_show(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //gt9xx_open_test(i2c_client_point);
    gt9xx_rawdata_show(i2c_client_point);
...
static ssize_t rd_result_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
}
//end,shenhuijun 20140114 for rd_result
...
//add by xiaoguang.xiong,2013.11.24,for raw data show.
static DEVICE_ATTR(gtp_rawdata_show, S_IRUGO|S_IWUSR, gtp_sysfs_rawdata_show, gtp_sysfs_rawdata_store);
static DEVICE_ATTR(rawdata, S_IRUGO|S_IWUSR, gtp_sysfs_rawdata_show, gtp_sysfs_rawdata_store);
static DEVICE_ATTR(rd_result, S_IRUGO|S_IWUSR, rd_result_show, rd_result_store);
...
s32 gtp_test_sysfs_init(void)
{
    ...
    if (ret)
    {
        GTP_ERROR("%s: sysfs_create_version_file failed 1\n", __func__);
        return ret;
    }
    ret = sysfs_create_file(goodix_debug_kobj, &dev_attr_opentest.attr);
    if (ret)
    {
        GTP_ERROR("%s: sysfs_create_version_file failed 2\n", __func__);
        return ret;
    }

    //begin,xiaoguang.xiong,2013.11.24,for raw data show.
    ret = sysfs_create_file(goodix_debug_kobj, &dev_attr_gtp_rawdata_show.attr);
    if (ret)
    {
        GTP_ERROR("%s: sysfs_create_version_file failed 3\n", __func__);
        return ret;
    }
    //end,xiaoguang.xiong,2013.11.24,for raw data show.
...
s32 ctp_sysfs_init(void)
{
    s32 ret ;
    
    goodix_ctp_kobj = kobject_create_and_add("CTP", NULL) ;
    //SET_INFO_LINE_INFO("Starting initializing ctp_sysfs");
    if (goodix_ctp_kobj == NULL)
    {
        GTP_ERROR("%s: subsystem_register failed\n", __func__);
        return -ENOMEM;
    }
    ...
    ret = sysfs_create_file(goodix_ctp_kobj, &dev_attr_rawdata.attr);
    if (ret)
    {
        GTP_ERROR("%s: sysfs_create_version_file failed 3\n", __func__);
        return ret;
    ...
    ret = sysfs_create_file(goodix_ctp_kobj, &dev_attr_rd_result.attr);
    if (ret)
    {
        GTP_ERROR("%s: sysfs_create_version_file failed 3\n", __func__);
        return ret;
...
void ctp_sysfs_deinit(void)
{
    sysfs_remove_file(goodix_ctp_kobj, &dev_attr_rawdata.attr);
    sysfs_remove_file(goodix_ctp_kobj, &dev_attr_rd_result.attr);
    kobject_del(goodix_ctp_kobj);
}
