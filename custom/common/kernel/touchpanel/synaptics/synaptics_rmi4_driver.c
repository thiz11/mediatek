...
//Tinno:CJ diff config for debug and user
...
static int tpd_def_calmat[8] = TPD_CALIBRATION_MATRIX;
#endif
	...
	while (left_len > 0) {
		...
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			...
			if (left_len > 8) {
				...
				#ifdef  SYNAPTICS_DEBUG_IF
					if(left_len>MAX_DATA_READ){
						len_temp=MAX_DATA_READ;
						retval = i2c_master_send(client, pData, 1);
						client->addr  = (client->addr  & I2C_MASK_FLAG) | I2C_DMA_FLAG;
						retval =i2c_master_recv(client, tpDMABuf_pa, (len_temp<<8 |1));
						client->addr   = client->addr & I2C_MASK_FLAG;
						memcpy((void *)pData, tpDMABuf_va, MAX_DATA_READ);
						pData=pData+MAX_DATA_READ;
					}else{
						retval = i2c_master_send(client, pData, 1);
						client->addr  = (client->addr  & I2C_MASK_FLAG) | I2C_DMA_FLAG;
						retval =i2c_master_recv(client, tpDMABuf_pa, (left_len<<8 |1));
						client->addr   = client->addr & I2C_MASK_FLAG;
						memcpy((void *)pData, tpDMABuf_va, left_len);
					}
				#else
...
#ifdef  SYNAPTICS_DEBUG_IF
		left_len=left_len-MAX_DATA_READ;
#else
		left_len -= 8;
		pData += 8;
		tmp_addr += 8;
...
#ifdef  SYNAPTICS_DEBUG_IF
	printk("tpd_i2c_read_data ret=%d length=%d \n",retval,length);
	if(length<retval)
	{
		retval=length;
	}
...

{
	...
	while (left_len > 0) {	
		...
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			...
			#endif	
				#ifdef  SYNAPTICS_DEBUG_IF
					memcpy(tpDMABuf_va,(void *)pData,  left_len);
					client->addr = (client->addr & I2C_MASK_FLAG) | I2C_DMA_FLAG;
					retval = i2c_master_send(client, tpDMABuf_pa, left_len);
					client->addr = client->addr & I2C_MASK_FLAG;
				#else
...
exit:
	mutex_unlock(&(ts->io_ctrl_mutex));
#ifdef  SYNAPTICS_DEBUG_IF
	printk("tpd_i2c_write_data ret=%d length=%d \n",retval,length);
	if(length<retval)
	{
		retval=length;
	}
...
static void tpd_up(int x, int y ,int id)
{
	...
	input_mt_sync(tpd->dev);
		...
		input_report_abs(tpd->dev, ABS_MT_PRESSURE, 0);
	        input_report_key(tpd->dev, BTN_TOUCH, 0);
/*
        #if defined (TINNO_S9320AE_PHONE_720P)
        input_report_abs(tpd->dev, ABS_MT_POSITION_X, x*1080/720);
    	input_report_abs(tpd->dev, ABS_MT_POSITION_Y, y*1920/1280);
        #else
		input_report_abs(tpd->dev, ABS_MT_POSITION_X, x);
		input_report_abs(tpd->dev, ABS_MT_POSITION_Y, y);
        #endif  // TINNO_S9320AE_PHONE_720P 
	//	input_report_abs(tpd->dev, ABS_MT_TRACKING_ID, id);
*/
		input_mt_sync(tpd->dev);
#endif	
	...
	if (status & ts->fn1a_mask) { /* key */
		...
		//	  #ifdef MULTI_TOUCH_PROTOCOL_TYPE_B
			  ...
			  if (FACTORY_BOOT == boot_mode || RECOVERY_BOOT == boot_mode)
			  {
				...
				tpd_down(175,2100,50,SYNAPTICS_TOUCH_TRACK_IDS-1);
			  }
		//	  #else
		//	  	tpd_down(175,2100,50);
		//	  #endif
			  	id = i;
			  }
			  if((button & (0x01 << i))==2){
			...
			if (FACTORY_BOOT == boot_mode || RECOVERY_BOOT == boot_mode)
			  {
					...
					tpd_down(550,2100,50,SYNAPTICS_TOUCH_TRACK_IDS-1);
			  }
			  
		//	  #else
		//	  	tpd_down(550,2100,50);
		//	  #endif
			  	id = i;
			  }
			  if((button & (0x01 << i))==4){
			...
			//  #ifdef MULTI_TOUCH_PROTOCOL_TYPE_B
			  if (FACTORY_BOOT == boot_mode || RECOVERY_BOOT == boot_mode)
			  {
				input_report_key(tpd->dev, touch_key_array[i], button & (0x01 << i));
					...
					tpd_down(890,2100,50,SYNAPTICS_TOUCH_TRACK_IDS-1);
			  }
			//  #else
			//  	tpd_down(890,2100,50);
			//  #endif
			  	id = i;
			  }
			}
...
//	if (status & 0x04) { /* point */
	if (status & ts->fn11_mask) {	
		...
		//	TPD_DMESG("index =%d ,ppt->status :%d \n",i,ppt->status);
...
#endif				  
			    #if 0
			    if(G_point[i].down_count<5)
			    {
	                            if(G_point[i].status)
	                            {
						if(((G_point[i].x-ppt->x)*(G_point[i].x-ppt->x)+(G_point[i].y-ppt->y)*(G_point[i].y-ppt->y))<2500)
						{
							ppt->x=G_point[i].x;
							ppt->y=G_point[i].y;
						}else{
...
void Config_Touch_GPIO(bool  work_or_stop)
{
   if(work_or_stop)//work
   {
	...
	mt65xx_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, CUST_EINT_TOUCH_PANEL_POLARITY, tpd_eint_handler, 0); 
#else
	mt_eint_set_hw_debounce(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_CN);
	mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_TYPE, tpd_eint_handler, 0);
#endif
	mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
...
//	struct synaptics_rmi4_f1a_handle *f1a;
 ...
 //   mt_set_gpio_out(GPIO_CTP_EN_PIN, GPIO_OUT_ONE);
       hwPowerOn(MT6322_POWER_LDO_VGP1, VOL_2800, "TP");
...
err_tpd_data:
	...
	kfree(ts);
    hwPowerDown(MT6322_POWER_LDO_VGP1, "TP");
	return retval;
}
