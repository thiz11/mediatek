...
#if ((defined MT6573) || (defined MT6575) || (defined MT6577) || (defined MT6589) || (defined MT6572) || (defined MT6582) || (defined MT6592))
...
#define STK_LT_N_CT			(170)
#define STK_HT_N_CT			(290)
...
/*----------------------------------------------------------------------------*/
static int stk3x1x_chk_reg_valid(struct i2c_client *client)
{
	struct stk3x1x_priv *obj = i2c_get_clientdata(client);
	int ret = 0;
	u8 buf[2];
	...
	ret = stk3x1x_master_recv(client, STK_INT_REG, buf, 0x02);
	if(ret < 0)
	{
		APS_DBG("error: %d\n", ret);
		return -EFAULT;
	...
	if(buf[0] != obj->int_val)
	{
		APS_ERR("%s: invalid reg 0x04=0x%2x\n", __func__, buf[0]);
		return 0xFF;
	}
	if(buf[1] != obj->wait_val)
	{
...
static int stk3x1x_validate_n_handle(struct i2c_client *client)
{
	...
	err = stk3x1x_chk_reg_valid(obj->client);
	if(err < 0)
	{
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if(ret != 0)
	{
		printk("xl_debug:stk3x1x_ps_tune_zero_val ret != 0\n");
	}
	if(ret == 0)
	{
...
#endif
		hrtimer_cancel(&obj->ps_tune0_timer);
		//	printk("xl_debug, %s %d : queue_count = %d\n", __func__, __LINE__, queue_count);
		//	queue_count = 0;
	}
...
static ssize_t stk3x1x_show_config(struct device_driver *ddri, char *buf)
{
	...
	res = scnprintf(buf, PAGE_SIZE, "(%d %d %d %d %d %d)\n",
			...
			atomic_read(&stk3x1x_obj->ps_mask), atomic_read(&stk3x1x_obj->ps_high_thd_val),
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				{
					atomic_set(&obj->ps_high_thd_val, MAX_THRESHOLD);
					atomic_set(&obj->ps_low_thd_val, MAX_THRESHOLD);
					if((err = stk3x1x_enable_ps(obj->client, 0, 1)))
					{
						APS_ERR("disable ps fail: %d\n", err);
						return -1;
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	}while(--i && !obj->stk_ps_tune0_wq);
	if(!obj->stk_ps_tune0_wq)
	{
		printk("workqueue create failure!");
		//		goto WQ_CREATE_ERR;
	}
...
static int stk3x1x_probe(struct platform_device *pdev) 
{
    ...
    /*dynamic adding of dev to have a oppo for HIFI*/
    struct i2c_adapter* adp;
    struct i2c_client* clt;
    adp = i2c_get_adapter(2);
    if (adp) {
        clt = i2c_new_device(adp, &i2c_stk3x1x);
        if (! clt)
            printk(KERN_ERR "ALSPS: can not add stk3x1x device\n");
    }
#if 0
