...
kal_uint16 OV5648MIPI_check_otp_wb(kal_uint16 index)
{
   ...
   if (index == 1)   //group 1
	...
	mDELAY(5);
	flag = OV5648MIPI_read_cmos_sensor(0x3d05);//MID 
	...
	OV5648MIPI_write_cmos_sensor(0x3d81, 0x01);
	mDELAY(5);
	flag = OV5648MIPI_read_cmos_sensor(0x3d0e);//MID
    //read otp bank 1
    OV5648MIPI_write_cmos_sensor(0x3d84,0xc0);	
	...
	msleep(50); // delay 50ms
	flag = OV5648MIPI_read_cmos_sensor(0x3d07);//MID
 ...
 if (index == 1)
  {
	...
	(*otp_ptr).user_data[2] = OV5648MIPI_read_cmos_sensor(0x3d0c);
  }
    else if(index == 2)  //Group 2
    {
	// read otp --Bank 0
	...
	(*otp_ptr).user_data[2] = OV5648MIPI_read_cmos_sensor(0x3d05);
    }
    else if(index == 3) //Group 3
    {
    //read otp bank 1
	...
	(*otp_ptr).user_data[2] = OV5648MIPI_read_cmos_sensor(0x3d0e);
...
{	
	...
	for(i=1;i<=3;i++) 
	{
    temp = OV5648MIPI_check_otp_wb(i);
...
static void OV5648MIPI_Sensor_Init(void)
{
    ...
    */
    ...
    OV5648MIPI_write_cmos_sensor(0x3013, 0x08);//0x04-->0x00-->0x08  Turn off internal LDO
