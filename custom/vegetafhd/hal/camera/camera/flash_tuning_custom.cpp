...
FLASH_PROJECT_PARA& cust_getFlashProjectPara(int aeMode, NVRAM_CAMERA_STROBE_STRUCT* nvrame)
{
	...
	para.engLevel.pfDuty = 1; //1;//modified by sen.luo 预闪设为300ma
	para.engLevel.mfDutyMax = 2; //2;//modified by sen.luo 驱动层当duty为2时 主闪设为700ma
	para.engLevel.mfDutyMin = 2; //2;//sen.luo 如果设置大于2时，对黑暗处 拍照会死机
	...
	para.coolTimeOutPara.tabId[2]=2;//dutyNum=3, so duty max=2
