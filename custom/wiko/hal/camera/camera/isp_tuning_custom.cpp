...
{
#ifdef MTK_FACE_COLOR
    //mike.wang ,DATE20140422, WIKOKK-385, 1-1, START
    MY_LOG("Mike->Sesor = %d", getSensorDev());
    if(getSensorDev()==0x01){
        MINT32 CCM_00 = CONV_2COMP_11(rCCM.conv0a.bits.G2G_CNV_00);
        if(CCM_00 > 362)
        {
            rCCM.set[0] = 0x07A4016B;
            rCCM.set[1] = 0x000007F2;
            rCCM.set[2] = 0x01AF07BD;
            rCCM.set[3] = 0x00000795;
            rCCM.set[4] = 0x074E0002;
            rCCM.set[5] = 0x000001B0;
        }
    }
    //mike.wang ,DATE20140422, WIKOKK-385, 1-1, END
#endif
...
{
/*
		if(rCamInfo.rAEInfo.i4LightValue_x10 < LOWLV )
		{
				rANR.set[9] = 0x00831F0A;
				rANR.set[10] = 0xAE77522C;
		}
		*/
    #if 0
    MY_LOG("%s()\n", __FUNCTION__);
...
refine_CCR(RAWIspCamInfo const& rCamInfo, IspNvramRegMgr const& rIspRegMgr, ISP_NVRAM_CCR_T& rCCR)
{
	/*
		if(rCamInfo.rAEInfo.i4LightValue_x10 < LOWLV )
		{
				rCCR.set[0] = 0xE0000100;
				rCCR.set[1] = 0x3A0C5646;
				rCCR.set[1] = 0x400F6300;
				rCCR.set[1] = 0x4019190F;
		}
		*/
    #if 0
    MY_LOG("%s()\n", __FUNCTION__);
