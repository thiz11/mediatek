...
u32 pinSet[2][8] = {
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV9760_MIPI_RAW,currSensorName)))
		{
				PK_DBG("kdCISModulePowerOn get in---  SENSOR_DRVNAME_OV9760_MIPI_RAW \n");
				PK_DBG("[ON_general 2.8V]sensorIdx:%d \n",SensorIdx);
				if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800,mode_name))
				{
					PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
...
//       if(pinSetIdx == 0) {   //v0.3
	 if(1) {                     //V0.4 hardware bom
        if(mt_set_gpio_mode(GPIO_CAMERA_LDO_A280V_EN_PIN, GPIO_CAMERA_LDO_EN_PIN_M_GPIO)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN mode failed!! \n");}
        if (mt_set_gpio_pull_enable(GPIO_CAMERA_LDO_A280V_EN_PIN, GPIO_PULL_DISABLE)) {PK_DBG("[[CAMERA SENSOR] Set CAMERA_POWER_PULL_PIN DISABLE ! \n"); }
	 if(mt_set_gpio_dir(GPIO_CAMERA_LDO_A280V_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN dir failed!! \n");}
        if(mt_set_gpio_out(GPIO_CAMERA_LDO_A280V_EN_PIN,GPIO_OUT_ONE)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN failed!! \n");}
    	}
	else{
        //AVDD
        if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_A, VOL_2800,mode_name))
        ...
        if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_IMX135_MIPI_RAW,currSensorName))){
		        PK_DBG("Set CAMERA_POWER_1.05vDVDD_PIN for power \n");    
		if(mt_set_gpio_mode(GPIO_CAMERA_LDO_D105V_EN_PIN, GPIO_CAMERA_LDO_EN_PIN_M_GPIO)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN mode failed!! \n");}
		if (mt_set_gpio_pull_enable(GPIO_CAMERA_LDO_D105V_EN_PIN, GPIO_PULL_DISABLE)) {PK_DBG("[[CAMERA SENSOR] Set CAMERA_POWER_PULL_PIN DISABLE ! \n"); }
		if(mt_set_gpio_dir(GPIO_CAMERA_LDO_D105V_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN dir failed!! \n");}
		if(mt_set_gpio_out(GPIO_CAMERA_LDO_D105V_EN_PIN,GPIO_OUT_ONE)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN failed!! \n");}
	}
	else{
		 PK_DBG("[CAMERA SENSOR] Set CAMERA_POWER_1.2vDVDD_PIN for power \n");
	        if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1200,mode_name))
	        {
	             PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
	             //return -EIO;
...
/*		if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_IMX135_MIPI_RAW,currSensorName))){
		...
		{
			PK_DBG("kdCISModulePower--off get in---SENSOR_DRVNAME_OV9760_MIPI_RAW \n");
		...
		if(mt_set_gpio_mode(GPIO_CAMERA_LDO_A280V_EN_PIN, GPIO_CAMERA_LDO_EN_PIN_M_GPIO)){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
		if (mt_set_gpio_pull_enable(GPIO_CAMERA_LDO_A280V_EN_PIN, GPIO_PULL_DISABLE)) {PK_DBG("[CAMERA SENSOR] Set GPIO94 PULL DISABLE ! \n"); }
		if(mt_set_gpio_dir(GPIO_CAMERA_LDO_A280V_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");}
		if(mt_set_gpio_out(GPIO_CAMERA_LDO_A280V_EN_PIN,GPIO_OUT_ZERO)){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");}
 ...
 /*   	if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_A,mode_name)) {
       ...
       if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_IMX135_MIPI_RAW,currSensorName))){
	        PK_DBG("Set CAMERA_POWER OFF _1.05vDVDD_PIN\n");
  		if (mt_set_gpio_pull_enable(GPIO_CAMERA_LDO_D105V_EN_PIN, GPIO_PULL_DISABLE)) {PK_DBG("[CAMERA SENSOR] Set GPIO94 PULL DISABLE ! \n"); }
  		if(mt_set_gpio_mode(GPIO_CAMERA_LDO_D105V_EN_PIN, GPIO_CAMERA_LDO_EN_PIN_M_GPIO)){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
  		if(mt_set_gpio_dir(GPIO_CAMERA_LDO_D105V_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");}
  		if(mt_set_gpio_out(GPIO_CAMERA_LDO_D105V_EN_PIN,GPIO_OUT_ZERO)){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");}
	   ...
	   else
	{
		...
		PK_DBG("[CAMERA SENSOR] turn off all cammera power \n");
		mdelay(20);
	}
  }
	return 0;
