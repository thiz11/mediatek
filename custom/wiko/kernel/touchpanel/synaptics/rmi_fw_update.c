...
atomic_t Firmware_Update_Flag=ATOMIC_INIT(0);
...
extern void synaptics_chip_reset();
	...
	for (addr = PDT_START; addr > PDT_END; addr -= PDT_ENTRY_SIZE) {
		...
		if (rmi_fd.fn_number) {
			...
			case SYNAPTICS_RMI4_F34:
				printk("mtk-tpd addr=0x%x VALUE=0x%x \n",addr, rmi_fd.ctrl_base_addr);
				f34found = true;
				fwu->f34_fd = rmi_fd;
		#if defined(TINNO_ANDROID_S9320AE) /*force reg addr =0x4c !!!*/	
				fwu->f34_fd.ctrl_base_addr=0x4c;
		#endif
				fwu->intr_mask = 0;
...
//#define DEBUG_TP_UPDATE
#ifdef DEBUG_TP_UPDATE
extern int idle_delay_sencond;
#endif
...
#else
	retval = fwu_wait_for_idle(5*ENABLE_WAIT_MS);
#endif
	if (retval < 0)
		return retval;
...
//add end
	...
	if (f01_device_status.flash_prog) {
		...
		retval = -1;
		TPD_DMESG("f01_device_status.flash_prog exit \n");
		goto exit;
	}
...
int synaptics_fw_version_updater()
{
	int retval=0;
	unsigned char config_id[4];
    ...
    printk("fwu->f34_fd.ctrl_base_addr=0x%x \n",fwu->f34_fd.ctrl_base_addr);
 printk("config_id =0x%x  0x%x 0x%x 0x%x \n",config_id[0],config_id[1],config_id[2],config_id[3]);
    if(retval<0)
    {
	tpd_fw_version =0;
	...
	tpd_fw_version =device_configID;
	sprintf(tpd_desc, "JieMian-S9320AE-S3202-%d", tpd_fw_version);
     	}
	////////////////////////////////////////////////////////////////////////////
	return retval;
}
EXPORT_SYMBOL(synaptics_fw_version_updater);
#endif
...
int synaptics_fw_updater(unsigned char *fw_data)
{
	...
	if(image_configID != device_configID){ /*test tp update*/
		...
		char *buf, loff_t pos, size_t count)
{
	...
	fwu->data_pos += count;
      ...
      printk("fwu_sysfs_store_image count=%d  fwu->data_pos =%d \n",count,fwu->data_pos);
	return count;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	{
	   printk("err _lock Firmware_Update_Flag \n");
          goto exit;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	fwu->data_pos = 0;
       ...
       printk("fwu_sysfs_image_size_store fwu->image_size = %d \n",fwu->image_size);
