...
#if (V_CHARGER_COMPATIBLE_ENABLE == 1)
static int charger_compatible_500mA_counter = 0; 
static int g_is_500mA_charger = 0;
#endif
...
#if defined (ACER_C11) //for 68 mo, if g_low_power_ready = 1, Iterm = 97mA else Iterm = 194mA  
int g_low_power_ready = 1;
#else
...
void select_charging_curret_bq24158(void)
{
    ...
    {    
       ...
       {
         if (g_is_500mA_charger == 1)
         {
                bq24158_config_interface_reg(0x01,0x78);//input current limit 500mA
		 } 
	   }
 #endif
...
void pchr_turn_on_charging_bq24158(void)
{
    ...
    if ((BMT_status.charger_type == STANDARD_CHARGER) ||(BMT_status.charger_type == CHARGING_HOST))
    {          
        if (BMT_status.charger_vol <= V_CHARGER_COMPATIBLE_V )
        {
            charger_compatible_500mA_counter++;			
        }
		else
		{
            charger_compatible_500mA_counter = 0;
		}	
       
        if(charger_compatible_500mA_counter >= AC_CHARGER_COMPATIBLE_CNT)
        {
			charger_compatible_500mA_counter = 0;
            g_is_500mA_charger = 1;								
        }		
    }
#endif
...
int g_Get_I_Charging(void)
{
    ...
    {
    #if defined(ACER_C11)
        ICharging = (ADC_I_SENSE - ADC_BAT_SENSE)*1000/56; 
    #else
...
int BAT_CheckBatteryStatus_bq24158(void)
{
    ...
    if( upmu_is_chr_det() == KAL_TRUE)
    {
	    ...
	    {
				charger_high_counter++;
		}
		else
		{
				charger_high_counter = 0;
		}
		
		if(charger_high_counter > 4)
        {
       ...
       else if(BMT_status.charger_vol < V_CHARGER_MAX )
        {
            ...
            {
			xlog_printk(ANDROID_LOG_WARN, "Power/Battery", "[BATTERY]Charger under ovp voltage recovery!!\r\n");                    
			BMT_status.charger_protect_status = 0;
			BMT_status.bat_charging_state = CHR_PRE;
            }
        }
...
void mt_battery_notify_check(void)
{
    ...
    if(g_BN_TestMode == 0x0000)
    {
  ...
  #endif			
		{
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] bat_temp(%d) <= -10'C\n", BMT_status.temperature);
		}
		else
		{
			g_BatteryNotifyCode &= ~(0x0020);
