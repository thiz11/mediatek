...
int CHARGING_FULL_CURRENT=150;    // mA 
...
int BAT_CheckBatteryStatus(void)
{
	...
	if(charger_low_counter>4)

        {
			...
			//printk("tengdeqiang [BATTERY]Charger under voltage!!\r\n");
			charger_low_counter=0;
			BMT_status.charger_protect_status =BATTERY_UNDER_VOL; 
            BMT_status.bat_charging_state = CHR_ERROR;
            return PMU_STATUS_FAIL;        
        }
	...
	if(charger_high_counter > 4)
		...
		//printk("tengdeqiang BATTERY]Charger over voltage !!\r\n!\r\n");
		charger_high_counter = 0;
