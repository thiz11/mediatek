...
#ifdef USING_TFA9890_EXTAMP
static const char* tfa9890_dev="/dev/i2c_tfa9887L";
static uint8_t tfa9890_isfound=0;
uint8_t Tfa9890_IsFound(void)
{
   return tfa9890_isfound==0?1:0;
}
uint8_t Tfa9890_Detect(void)
{
    int fd = open(tfa9890_dev, O_RDONLY);
    if (fd < 0)
    {
        ALOGD("Couldn't open '%s' (%s)", tfa9890_dev, strerror(errno));
        return -1;
    }
    close(fd);
    return 0;
...
status_t AudioMTKHardware::HardwareInit(bool bEnableSpeech)
{
...
#if defined(NXP_SMARTPA_SUPPORT)
    ALOGD("Turn on 2nd I2S_IN/2nd I2S_OUT path for tfa98xx, using for loading DSP profiles, only once.");
    ...
    mAudioDigitalInstance->SetMemIfEnable(AudioDigitalType::I2S_IN_2, true);
    mAudioDigitalInstance->Set2ndI2SInConfig(44100, AudioDigtalI2S::MASTER_MODE);

//    MTK_Tfa98xx_Init();  //mask for test
#elif defined(USING_TFA9887_EXTAMP)
	// turn on I2S_OUT_2 path for tfa9887, using for loading DSP profiles
	...
	//add for tfa9887 init
	tfa9887_init();
#elif defined(USING_TFA9890_EXTAMP)
	tfa9890_isfound=Tfa9890_Detect();
        ALOGD("%s:L%d,Tfa9890_Detect=%d\n",__func__,__LINE__,tfa9890_isfound);
        if(Tfa9890_IsFound())
	{
	// turn on I2S_OUT_2 path for tfa9887, using for loading DSP profiles
	ALOGD("Turn on I2S_OUT_2 path for tfa9887, using for loading DSP profiles, only once.");
...
status_t AudioMTKHardware::setParameters(const String8 &keyValuePairs)
{
    ...
    do
    {
        ...
        // Low latency mode
        if (param.get(keySET_KERNEL_DEBUG_MODE, value_str) == NO_ERROR)
        {
            param.remove(keySET_KERNEL_DEBUG_MODE);
            //Diable low latency mode for 32bits audio task
            setKernelDebugMode(value_str == "1");
            break;
        }
...
status_t AudioMTKHardware::setMode(int NewMode)
{
    ...
    // mMode is previous
    if (ModeInCall(mMode)) { pSpeechPhoneCallController->CloseModemSpeechControlFlow(mMode); }
    if (ModeInCall(new_mode)) { pSpeechPhoneCallController->OpenModemSpeechControlFlow(new_mode); }
...
void AudioMTKHardware::setLowLatencyMode(bool mode)
{
    static Mutex mLowLatencyModeLock;
    Mutex::Autolock _l(mLowLatencyModeLock);

    isLowLatencyMode = mode;
    String8 para = String8("LowLatencyMode=") + String8(isLowLatencyMode ? "1" : "0");
    mAudioMTKStreamManager->setParametersToStreamOut(para);
}

void AudioMTKHardware::setKernelDebugMode(bool inDebugMode)
{
    static Mutex mLowLatencyModeLock;
    String8 para = String8("KernelDebugMode=") + String8(inDebugMode ? "1" : "0");
    mAudioMTKStreamManager->setParametersToStreamOut(para);
}
