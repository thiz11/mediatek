...
static unsigned int hdmi_resolution_param_table[][3] =
{
        ...
        {1920,  1080,   60},
        {1920,  1080,   50},
        #elif defined(MTK_INTERNAL_HDMI_SUPPORT) ||defined(MTK_INTERNAL_MHL_SUPPORT)
		{720,   480,    60},
        {720,   576,    50},
        {1280,  720,    60},
        {1280,  720,    50},
    ...
    for( ;; ) {
           ...
           !(needStopWdma1 && hdmi_overlay_status == HDMI_OVERLAY_STATUS_STOPPED))
        {
            ...
            else
            {
                ...
                addr = temp_mva_w + hdmi_buffer_lcdw_id_tmp * p->lcm_width * p->lcm_height * 3;
...
static void hdmi_update_impl(void)
{
    ...
    if(temp_va != 0 && hdmi_va != 0)
    {
        ...
        pddp.srcAddr[0] = temp_va + p->lcm_width * p->lcm_height * 3 * ovlReadBuffer;
...
#else
		pddp.orientation = p->orientation + 270;
#endif
        ...
        if(p->lcm_is_video_mode)
        {
            ...
            if(dequeuedBuffer != -1)
            {
                ...
                if( DDPK_Bitblt_Config(DDPK_CH_HDMI_0, &pddp) )
                {
                    HDMI_LOG("[HDMI] DDPK_Bitblt_Config return error, free such frame\n");
                    hdmi_release_buffer(dequeuedBuffer);
                }
                else if(DDPK_Bitblt(DDPK_CH_HDMI_0))
                {
                    HDMI_LOG("[HDMI] DDPK_Bitblt return error, free such frame\n");
                    hdmi_release_buffer(dequeuedBuffer);
                }
                else
                {
...
#ifdef HDMI_BUFFER_KEEP_USE
    HDMI_LOG("Skip Free temp_va and temp_mva\n");
#else
...
int hdmi_allocate_hdmi_buffer(void)
{
    ...
    {
        HDMI_LOG("HDMI Buffer is already allocated, hdmi_va=0x%08x, hdmi_mva_r=0x%08x\n", hdmi_va, hdmi_mva_r);
        return 0;
    }
#endif
...
{
#ifdef HDMI_BUFFER_KEEP_USE
    HDMI_LOG("SKIP Free hdmi_va and hdmi_mva, hdmi_va=0x%08x, hdmi_mva_r=0x%08x\n", hdmi_va, hdmi_mva_r);
#else
    int hdmi_va_size = p->hdmi_width * p->hdmi_height * hdmi_bpp * hdmi_params->intermediat_buffer_num;

    //free hdmi_va & hdmi_mva
    HDMI_LOG("Free hdmi_va and hdmi_mva, hdmi_va=0x%08x, hdmi_mva_r=0x%08x\n", hdmi_va, hdmi_mva_r);
...
int hdmi_rdma_address_config(bool enable, void* hdmi_mva)
{
    ...
    if(enable)
    {
        int rdmaInputFormat = hdmi_bpp == 2 ? RDMA_INPUT_FORMAT_UYVY : RDMA_INPUT_FORMAT_RGB888;
...
int hdmi_display_path_overlay_config(bool enable)
{
    ...
    if(enable)
    {
        int wdma1OutputFormat = eRGB888;

        unsigned int wdma1OutputAddr = temp_mva_w + p->lcm_width * p->lcm_height * 3 * hdmi_buffer_lcdw_id;
	...
	switch(arg)
	 {
	  ...
	  case HDMI_VIDEO_1280x720p_60Hz:
	  {
		 ...
		 clk_pol	 = HDMI_POLARITY_FALLING;
		 de_pol 	 = HDMI_POLARITY_RISING;
#if defined(HDMI_TDA19989)|| defined(MTK_INTERNAL_HDMI_SUPPORT) ||defined(MTK_INTERNAL_MHL_SUPPORT)
...
static long hdmi_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) && defined(MTK_HDMI_HDCP_SUPPORT)&&defined(MTK_DRM_KEY_MNG_SUPPORT))
	...
	printk("[PANDA][HDMI] hdmi ioctl= %s(%d), arg = %lu\n", _hdmi_ioctl_spy(cmd),cmd&0xff, arg);
    ...
    switch(cmd)
    {
	   ...
	   case MTK_HDMI_COLOR_DEEP:
       {
		 ...
		 #if (defined(MTK_INTERNAL_MHL_SUPPORT) || defined(MTK_INTERNAL_HDMI_SUPPORT))
	...
	case MTK_HDMI_COLOR_DEEP:
       ...
       case MTK_HDMI_AUDIO_VIDEO_ENABLE:
       {
        ...
        case MTK_HDMI_GET_DRM_ENABLE:
        {
            ...
            {
                HDMI_LOG("mutehdmi is null or disable %d \n", p->is_clock_on);
                drm_enable = 0;
            }
