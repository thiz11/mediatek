...
#define VENC_SW_PAUSE               VENC_BASE + 0x0AC
#define VENC_SW_HRST_N              VENC_BASE + 0x0A8
...
#define INFO_BASE_1     0x08000000
#define INFO_REGION_1   0x1000
#define INFO_BASE_2     0x10000000
#define INFO_REGION_2   0x1000
#define INFO_BASE_3     0x10004000
#define INFO_REGION_3   0x1000

int KVA_VENC_IRQ_ACK_ADDR;
int KVA_VENC_IRQ_STATUS_ADDR;
int KVA_VENC_SW_PAUSE, KVA_VENC_SW_HRST_N;
#ifdef VENC_PWR_FPGA
// Cheng-Jung 20120621 VENC power physical base address (FPGA only, should use API) [
...
void venc_power_on(void)
{
    ...
    enable_clock(MT_CG_IMAGE_VENC_JPENC, "VENC");
...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd) {
        case VCODEC_SET_THREAD_ID:
            MFV_LOGE("[MT6582] VCODEC_SET_THREAD_ID [EMPTY] + tid = %d\n", current->pid);
            
            MFV_LOGE("[MT6582] VCODEC_SET_THREAD_ID [EMPTY] - tid = %d\n", current->pid);
        break;

        case VCODEC_ALLOC_NON_CACHE_BUFFER:
            MFV_LOGE("[MT6582][M4U]! VCODEC_ALLOC_NON_CACHE_BUFFER [EMPTY] + tid = %d\n", current->pid);

            MFV_LOGE("[MT6582][M4U]! VCODEC_ALLOC_NON_CACHE_BUFFER [EMPTY] - tid = %d\n", current->pid);
        break;

        case VCODEC_FREE_NON_CACHE_BUFFER:
            MFV_LOGE("[MT6582][M4U]! VCODEC_FREE_NON_CACHE_BUFFER [EMPTY] + tid = %d\n", current->pid);            

            MFV_LOGE("[MT6582][M4U]! VCODEC_FREE_NON_CACHE_BUFFER [EMPTY] - tid = %d\n", current->pid);
        break;

        case VCODEC_INC_DEC_EMI_USER:
            MFV_LOGD("[MT6582] VCODEC_INC_DEC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_DEC_DEC_EMI_USER:
            MFV_LOGD("[MT6582] VCODEC_DEC_DEC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_INC_ENC_EMI_USER:
            MFV_LOGD("[MT6582] VCODEC_INC_ENC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_DEC_ENC_EMI_USER:
            MFV_LOGD("[MT6582] VCODEC_DEC_ENC_EMI_USER + tid = %d\n", current->pid);
        ...
        case VCODEC_LOCKHW:
            MFV_LOGD("[MT6582] VCODEC_LOCKHW + tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
            ...
            {
                VAL_UINT32_T u4VencLockTimeOutCount = 0;
                while (bLockedHW == VAL_FALSE)
                {
        ...
        case VCODEC_UNLOCKHW:
            MFV_LOGD("[MT6582] VCODEC_UNLOCKHW + tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
            ret = copy_from_user(&rHWLock, user_data_addr, sizeof(VAL_HW_LOCK_T));
            if (ret) {
        ...
        case VCODEC_INC_PWR_USER:
            MFV_LOGD("[MT6582] VCODEC_INC_PWR_USER + tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
        ...
        case VCODEC_DEC_PWR_USER:
            MFV_LOGD("[MT6582] VCODEC_DEC_PWR_USER + tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
            ret = copy_from_user(&rPowerParam, user_data_addr, sizeof(VAL_POWER_T));
            if (ret) {
        ...
        case VCODEC_WAITISR:
            ...
            MFV_LOGD("[MT6582] VCODEC_WAITISR + tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
            ...
            else if (val_isr.eDriverType == VAL_DRIVER_TYPE_H264_ENC)
            {
                ...
                if(VAL_RESULT_INVALID_ISR == eValRet)//time out
                {
                    int i;
                    for(i=0;i<5;++i)
                    {
        ...
        {
            MFV_LOGE("[MT6582] VCODEC_INITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
        ...
        { 
            MFV_LOGE("[MT6582] VCODEC_DEINITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
        ...
        case VCODEC_GET_CPU_LOADING_INFO:
        {
            ...
            MFV_LOGD("[MT6582] VCODEC_GET_CPU_LOADING_INFO +\n");
            user_data_addr = (VAL_UINT8_T *)arg;
            // TODO:
#if 1 // Morris Yang update for multi-core
            VAL_UINT8_T cpu_id;
        ...
        {
            MFV_LOGD("[MT6582] VCODEC_GET_CORE_LOADING + - tid = %d\n", current->pid);
        ...
        {
            MFV_LOGD("[MT6582] VCODEC_GET_CORE_NUMBER + - tid = %d\n", current->pid);
        ...
        case VCODEC_SET_CPU_OPP_LIMIT:
            MFV_LOGE("[MT6582] VCODEC_SET_CPU_OPP_LIMIT [EMPTY] + - tid = %d\n", current->pid);
            user_data_addr = (VAL_UINT8_T *)arg;
...
static int vcodec_open(struct inode *inode, struct file *file)
{
    ...
    MFV_LOGE("vcodec_open pid = %d, MT6582Driver_Open_Count %d\n", current->pid, MT6582Driver_Open_Count);
...
static int venc_hw_reset(int type)
{
    VAL_UINT32_T    uValue;
    VAL_RESULT_T    eValRet;
    MFV_LOGE("Start VENC HW Reset");

    ////clear irq event
    //EncIsrEvent.u4TimeoutMs = 1;
    //eValRet = eVideoWaitEvent(&EncIsrEvent, sizeof(VAL_EVENT_T));                
    //MFV_LOGE("ret %d", eValRet);
    ...
    eValRet = eVideoWaitEvent(&EncIsrEvent, sizeof(VAL_EVENT_T));                
    if(VAL_RESULT_INVALID_ISR == eValRet || gu4HwVencIrqStatus != VENC_IRQ_STATUS_PAUSE)//timeout or don't get pasue
    {//step 3
    ...
    {//step 4
        ...
        VDO_HW_WRITE(KVA_VENC_SW_PAUSE, 0);
        MFV_LOGE("4 HRST = %d, isr = 0x%x", uValue, gu4HwVencIrqStatus);
    }

    VDO_HW_WRITE(KVA_VENC_SW_HRST_N, 1);
    uValue = VDO_HW_READ(KVA_VENC_SW_HRST_N);
    MFV_LOGE("HRST = %d", uValue);
...
static int vcodec_flush(struct file *file, fl_owner_t id)
{
    ...
    mutex_lock(&DriverOpenCountLock);
    MFV_LOGE("vcodec_flush pid = %d, MT6582Driver_Open_Count %d\n", current->pid, MT6582Driver_Open_Count);
...
static int vcodec_mmap(struct file* file, struct vm_area_struct* vma)
{
    ...
    if(((length > VENC_REGION) || (pfn < VENC_BASE) || (pfn > VENC_BASE+VENC_REGION)) &&
       ...
       ((length > INFO_REGION_1) || (pfn < INFO_BASE_1) || (pfn > INFO_BASE_1+INFO_REGION_1)) &&
       ((length > INFO_REGION_2) || (pfn < INFO_BASE_2) || (pfn > INFO_BASE_2+INFO_REGION_2)) &&
       ((length > INFO_REGION_3) || (pfn < INFO_BASE_3) || (pfn > INFO_BASE_3+INFO_REGION_3))
      )
    {
...
static int vcodec_pm_restore_noirq(struct device *device)
{
    ...
    mt_irq_set_sens(MT_VDEC_IRQ_ID, MT_LEVEL_SENSITIVE);
    mt_irq_set_polarity(MT_VDEC_IRQ_ID, MT_POLARITY_LOW);
    // venc: IRQF_TRIGGER_LOW
    mt_irq_set_sens(MT_VENC_IRQ_ID, MT_LEVEL_SENSITIVE);
    mt_irq_set_polarity(MT_VENC_IRQ_ID, MT_POLARITY_LOW);
...
static int __init vcodec_driver_init(void)
{
    ...
    KVA_VENC_SW_PAUSE   = (int)ioremap(VENC_SW_PAUSE, 4);
    KVA_VENC_SW_HRST_N  = (int)ioremap(VENC_SW_HRST_N, 4);
#ifdef VENC_PWR_FPGA
...
static void __exit vcodec_driver_exit(void)
{
    ...
    iounmap((void*)KVA_VENC_SW_PAUSE);
    iounmap((void*)KVA_VENC_SW_HRST_N);
#ifdef VENC_PWR_FPGA
...
MODULE_DESCRIPTION("MT6582 Vcodec Driver");
MODULE_LICENSE("GPL");
