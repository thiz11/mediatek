...
int g_iTestMem_CPU0, g_iTestMem_CPU1, g_iTestMem_CPU2, g_iTestMem_CPU3;
...
#define DEFINE_SLT_CPU_MEMCPYL2_STORE(_N)    \
    DEFINE_SPINLOCK(mem_cpu##_N##_lock);    \
...
{   \
    ...
    for (i = 0, g_iCPU##_N##_PassFail = 0; i < g_iMemcpyL2LoopCount; i++) {    \
        spin_lock_irqsave(&mem_cpu##_N##_lock, cpu##_N##_flags);    \
        ret = fp3_memcpyL2_start(_N);    /* 1: PASS, 0:Fail, -1: target CPU power off */  \
        spin_unlock_irqrestore(&mem_cpu##_N##_lock, cpu##_N##_flags);   \
        if(ret != -1)   \
        {   \
