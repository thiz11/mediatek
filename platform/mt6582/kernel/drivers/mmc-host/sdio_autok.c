...
{
    unsigned int vcore_uv_offset = 0;
...
#endif
    AUTOK_PRINT("[%s] Current Vcore = 0x%x\r\n", __func__, vcore_uv_offset);
    
    return vcore_uv_offset;
}
...
#if defined(MT6582LTE)
static int autok_trans_bound_rising = 3;   /* 6/2 */
static int autok_trans_bound_falling = 3;  /* 6/2 */
#elif defined(MT6592LTE)
static int autok_trans_bound_rising = 1;   /* 3/2 */
static int autok_trans_bound_falling = 1;  /* 3/2 */
#endif
        ...
        /* Estimated 1T */  \
            ...
            if (isSRising)  \
                cyc = ABS_DIFF(char_s.pad_trans_e##s_num,   \
                    char_e.pad_trans_s##e_num + 2*AUTOK_TRANS_BOUND_RISING); \
            else            \
                cyc = ABS_DIFF(char_s.pad_trans_e##s_num,   \
                    char_e.pad_trans_s##e_num + 2*AUTOK_TRANS_BOUND_FALLING); \
        }           \
    }   \
    else    \
    {   \
        ...
        if(char_s.pad_trans_s##s_num && char_e.pad_trans_e##e_num)        \
            ...
            /* End full boundary, Start half boundary */    \
            if (isSRising)  \
                cyc = ABS_DIFF(char_e.pad_trans_m##e_num,   \
                    (char_s.pad_trans_e##s_num - AUTOK_TRANS_BOUND_RISING));    \
            else            \
                cyc = ABS_DIFF(char_e.pad_trans_m##e_num,   \
                    (char_s.pad_trans_e##s_num - AUTOK_TRANS_BOUND_FALLING));   \
        }   \
        else if(char_s.pad_trans_s##s_num && !char_e.pad_trans_e##e_num) {    \
            /* Start full boundary, End half boundary */    \
            if (isSRising)  \
                cyc = ABS_DIFF((char_e.pad_trans_s##e_num + AUTOK_TRANS_BOUND_FALLING), \
                    char_s.pad_trans_m##s_num);   \
            else            \
                cyc = ABS_DIFF((char_e.pad_trans_s##e_num + AUTOK_TRANS_BOUND_RISING),  \
                    char_s.pad_trans_m##s_num);    \
        }   \
        /* Estimated T/2 */  \
        else {   \
                ...
                if (CMDMatChar[CycScanSta].pad_trans_cnt1 &&
                    ...
                    if (AUTOK_CMD_EDGE_MATRIX_SCAN)
                        autok_trans_bound_falling = 
                            CMDMatChar[CycScanSta].pad_trans_cnt1/2;
                    else
                        autok_trans_bound_rising = 
                            CMDMatChar[CycScanSta].pad_trans_cnt1/2;

                    AUTOK_PRINT("Update trans. bound half len:%d(%s edge)\r\n", 
    ...
    P_AUTOK_CMD_TUNE_RES_T pCmdTuneRes)
{
    ...
    ChngBaseCK:
        ...
        if (!autok_rdata_scan[baseCKGEN].BoundReg1_S && 
            ...
            AUTOK_PRINT("Left boundary found at initial timing\r\n");
            ...
            if (DataMargin < (DataPassWin / 2)) {
                RealCKGEN = baseCKGEN + (((DataPassWin / 2) - DataMargin) * 2 / 
                                DIV_CEIL_FUNC(MIN_CLK_GEN_DELAY_IN_PS, MIN_PAD_DELAY_IN_PS));
            ...
            if (RealCKGEN > CKGENMax) {
                ...
                if (IntBound) 
                    AUTOK_PRINT("[WARN] The internal boundary is so large!\r\n");
                else
                    IntBound = 32;
                ...
                /* In case of right boundary by shifting CKGEN */
                    ...
                    AUTOK_PRINT("Right boundary by shifting CKGEN\r\n");
        ...
        /* In case of Right boundary at CKGEN = 0 */
            ...
            AUTOK_PRINT("Right boundary found at initial timing\r\n");
            ...
            if (RBoundPad >= (DataPassWin/2)) {
                ...
                if (IntBound) {
                    ...
                    if (RBoundPad > IntBound) {
                        ...
                        RealCKGEN = 0;
                        RealPadDelay = RBoundPad - (DataPassWin/2) - 1;
                    }
                    ...
                    /* To avoid the pad delay too large */
                    if (RealPadDelay >= 16) {
                        AUTOK_PRINT("[NOTICE] Too large pad gear(%d), shift to left by CKGEN\r\n", 
                            RealPadDelay);
                        
                        if (RealPadDelay >= (CKGENMax * 
                            DIV_CEIL_FUNC(MIN_CLK_GEN_DELAY_IN_PS, MIN_PAD_DELAY_IN_PS))) {

                            RealCKGEN = CKGENMax;
                            RealPadDelay -= CKGENMax * 
                                DIV_CEIL_FUNC(MIN_CLK_GEN_DELAY_IN_PS, MIN_PAD_DELAY_IN_PS);
                        }
                        else {
                            RealCKGEN = RealPadDelay/
                                DIV_CEIL_FUNC(MIN_CLK_GEN_DELAY_IN_PS, MIN_PAD_DELAY_IN_PS);
                            RealPadDelay = 0;
                        }
                    }
            ...
            if (RealCKGEN > CKGENMax) {
                ...
                /* 
                 * Considering another extreme case, that we can NOT
                 * configure T/2 margin for left and right boundary together 
                 * due to internal boundary, so after we find T/2 margin point,
                 * try to calculate the best margin at that situation.
                 * that is, sacrifying the left margin, to save the enough 
                 * margin for right
                 */
                if (IntBound) {
                     ...
                     * instead of internal boundary
                     */
                    if ((RealPadDelay >= autok_rdata_scan[RealCKGEN].BoundReg1_S) ||
                       ((RealPadDelay < autok_rdata_scan[RealCKGEN].BoundReg1_S) &&
                       ((autok_rdata_scan[RealCKGEN].BoundReg1_S - RealPadDelay) <= 
                        (DataPassWin/2 - AUTOK_TUNING_INACCURACY)))) {
...
#if defined(AUTOK_WDAT_BOUND_DOUB_CHK)
    fDoubleChk = 1;
#endif
...
static void autok_tuning_parameter_init(struct msdc_host *host, E_AUTOK_TUNING_STAGE stg, U_AUTOK_INTERFACE_DATA *pAutokData)
{
    ...
    if(stg == TUNING_STG1) {
        ...
        /* Read data pad delay */
        if (gfTinyMar) {
            /* In case of tiny margin, shift to right for 1 gear */
