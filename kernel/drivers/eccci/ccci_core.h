...
#define CCCI_MTU  (4096-128) //(3840-16)
...
typedef enum {
	...
	RX_IRQ, // broadcast by modem driver
}MD_STATE;
...
static void inline ccci_port_struct_init(struct ccci_port *port, struct ccci_modem *md)
{
	...
	port->modem = md;
	snprintf(wakelock_name, sizeof(wakelock_name), "%s_rx_wl", port->name);
	wake_lock_init(&port->rx_wakelock, WAKE_LOCK_SUSPEND, wakelock_name);
}
