...
#ifdef ROME
extern phys_addr_t *get_modem_start_addr_list(void);
extern int dfo_query(const char *s, unsigned long *v);
#else
phys_addr_t *get_modem_start_addr_list(void){return md_resv_mem_addr;}
int dfo_query(const char *s, unsigned long *v) {return 0;}
#endif
...
static void collect_md_mem_setting(void)
{
	...
	addr = get_modem_start_addr_list(); // MD ROM start address should be 32M align as remap hardware limitation
...
static void cal_md_mem_usage(void)
{
	...
	if(get_dfo_setting("MD1_SIZE", &tmp) == 0) {
		 ...
		 * for ECCCI: ROM+RAM size will be align to 1M on ROME, and share memory is 2K,
...
void ccci_md_mem_reserve(void)
{
	...
	phys_addr_t ptr = arm_memblock_steal(modem_size_list[MD_SYS1], CCCI_MEM_ALIGN);
	if(ptr) {
		md_resv_mem_addr[MD_SYS1] = ptr;
		CCCI_INF_MSG(-1, CORE, "md mem reserve successfully, ptr=0x%llx, size=0x%x\n", (unsigned long long)ptr, modem_size_list[0]);
	}else{
		CCCI_INF_MSG(-1, CORE, "md mem reserve fail.\n");
	}
}
