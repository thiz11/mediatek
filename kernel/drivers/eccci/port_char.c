...
#ifdef MT6290
int get_sim_type(struct ccci_modem *md, int *p_sim_type){return 0;}
int switch_sim_mode(int id, char *buf, unsigned int len){return 0;}
unsigned int get_sim_switch_type(void){return 0;}
#else
extern int get_sim_type(struct ccci_modem *md, int *p_sim_type);
...
static int dev_char_open(struct inode *inode, struct file *file)
{
	...
	if(atomic_read(&port->usage_cnt))
		return -EBUSY;
	CCCI_INF_MSG(port->modem->index, CHAR, "port %s open with flag %X\n", port->name, file->f_flags);
...
static int dev_char_close(struct inode *inode, struct file *file)
{
	...
	CCCI_INF_MSG(port->modem->index, CHAR, "close CH%u rx_len=%d empty=%d\n", port->rx_ch, 
		port->rx_length, list_empty(&port->rx_req_list));
	// 2. decrease usage count and check critical nodes for reset
...
static ssize_t dev_char_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if(port->tx_ch == CCCI_MONITOR_CH) // currently, virtual char does NOT support writing
		return -EPERM;
...
static long dev_char_ioctl( struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	unsigned int sim_mode, sim_switch_type, enable_sim_type;
#ifdef MT6290
...
#define CCCI_IOC_CLR_HEADER             _IO(CCCI_IOC_MAGIC,  22)
	...
	case CCCI_IOC_GET_MD_EX_TYPE:
		ret = md->ex_type;
		CCCI_INF_MSG(md->index, CHAR, "get modem exception type=%d\n", ret);
		break;
...
drop:
	// drop this packet
	CCCI_ERR_MSG(port->modem->index, CHAR, "dropping on CH%u length %d\n", port->rx_ch, port->rx_length);
	list_del(&req->entry); 
	req->policy = RECYCLE;
	ccci_free_req(req);
