...
static void control_msg_handler(struct ccci_port *port, struct ccci_request *req)
{
	...
	CCCI_DBG_MSG(md->index, KERN, "control message %X\n", ccci_h->data[1]);
	if(ccci_h->data[1] == MD_INIT_START_BOOT 
		...
		if (unlikely(ccci_h->reserved != MD_EX_CHK_ID)) {
			...
			del_timer(&md->bootup_timer);
			mod_timer(&md->ex_monitor,jiffies+HZ);
		...
		if (unlikely(ccci_h->reserved!=MD_EX_REC_OK_CHK_ID || req->skb->len<(sizeof(struct ccci_header)+sizeof(EX_LOG_T)))) {
			...
			if(need_update_state) {
				// should not run into here, as exception IRQ has set MD_STATE_UPDATE
...
err2:
	...
	case IPC_RPC_GET_EINT_ATTR_OP:	
	{
		...
		res = p_rpc_buf->buffer + 4*sizeof(unsigned int);
...
static void ccci_ee_info_dump(struct ccci_modem *md, DEBUG_INFO_T *debug_info)
{
	...
	*/
	...
	CCCI_DBG_MSG(md->index, KERN, "Dump MD%d Image memory\n", md->index);
	ccci_mem_dump((void *)md->mem_layout.md_region_vir, MD_IMG_DUMP_SIZE);
	// Dump MD image memory
	CCCI_DBG_MSG(md->index, KERN, "\n\n");
	CCCI_DBG_MSG(md->index, KERN, "Dump MD%d layout struct\n", md->index);
...
void md_ex_monitor_func(unsigned long data)
{
	...
	if ((ee_info_flag&((1<<MD_EE_MSG_GET)|(1<<MD_EE_OK_MSG_GET))) == \
		...
		CCCI_DBG_MSG(md->index, KERN, "Receive MD_EX_REC_OK\n");
		...
		if((ee_info_flag&(1<<MD_STATE_UPDATE))==0){
			need_update_state = 1;
			CCCI_DBG_MSG(md->index, KERN, "MD no response > 1500ms.\n");
		}
	} else { 
		CCCI_DBG_MSG(md->index, KERN, "Invalid MD_EX\n");
		goto _dump_done;
	}
...
void md_bootup_timeout_func(unsigned long data)
{
	...
	CCCI_DBG_MSG(md->index, KERN, "Dump MD Image memory\n");
	ccci_mem_dump((void*)md->mem_layout.md_region_vir, MD_IMG_DUMP_SIZE);
	...
	if(md->boot_stage == MD_BOOT_STAGE_0) {
		...
		ccci_mem_dump((int*)md_ctlb[md_id]->smem_table->ccci_exp_smem_base_virt, MD_EX_LOG_SIZE);
		*/
		ccci_aed(md, CCCI_AED_DUMP_EX_MEM, ex_info);
	}
}
