...
/*Punk@add*/
#define LCD_INFO_PROC

#ifdef LCD_INFO_PROC
#define LCD_INFO_PROC_FILE    "driver/lcd_info"
static struct proc_dir_entry *lcd_info_proc = NULL;
#endif
...
#ifdef LCD_INFO_PROC	/*Punk@add*/

#if defined(ACER_C11)
extern int IMM_GetOneChannelValue(int dwChannel, int data[4], int* rawdata);
#endif

extern char* disp_get_lcm_name(void);
static int lcd_info_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    char *ptr = page;
	char *name;
#if defined(ACER_C11)
...
static int mtkfb_probe(struct device *dev)
{
	...
	else
	{
		...
		LCD_CHECK_RET(LCD_Init());
			...
			lcd_info_proc = create_proc_entry( LCD_INFO_PROC_FILE , 0666, NULL);

			if ( lcd_info_proc == NULL )
			{
				printk("create_proc_entry %s failed\n", LCD_INFO_PROC_FILE );
			}
			else 
			{
				lcd_info_proc ->read_proc = lcd_info_read_proc;
				lcd_info_proc ->write_proc = lcd_info_write_proc;
			}
#endif
